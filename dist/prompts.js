 ...
 */
async function promptForAllInputs(imageUris) {
...
async function promptForOutputDirectory() {
    ...
    const staticContentFolder = workspaceFolders.find((folder) => folder.name.toLowerCase() === 'wwwroot' || folder.name.toLowerCase() === 'public');
...
async function promptForSizes() {
    ...
    return selectedSizes ? selectedSizes.map((size) => parseInt(size, 10)) : undefined;
}
